...
pipeline {
    ...
    environment {
        ...
        DOCKER_IMAGE = 'lmoisei/api-gateway'
...
stage('Checkout') {
    steps {
        script {
            if (fileExists('upskilling-api-gateway/.git')) {
                dir('upskilling-api-gateway') {
                    sh('git pull')
                    COMMIT_HASH = sh(script: 'git rev-parse HEAD', returnStdout: true).trim()
                    echo "COMMIT_HASH is: ${COMMIT_HASH}" // Print the commit hash
                }
            } else {
                sh('git clone https://github.com/MoiseiLiviu/upskilling-api-gateway.git')
                dir('upskilling-api-gateway') {
    ...
    steps {
        script {
            withCredentials([file(credentialsId: 'kubeconfig', variable: 'KUBECONFIG')]) {
                ...
                sh("kubectl set image deployment/api-gateway api-gateway=${DOCKER_IMAGE}:${COMMIT_HASH} --record")
